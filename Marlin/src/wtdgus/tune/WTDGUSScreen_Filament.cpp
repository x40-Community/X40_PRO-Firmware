...
void DGUS_Screen_Filament::Update()
{
	...
	case FSSE_HEATTING:
		...
		else if (filamentType == FT_CPE)
		{
			dserial.SendString(ADDR_FILAMENT_TEXT_TEMP, MMSG_FILAMENT_TYPE_TEMP6, 8);
            if (act_extruder == ACTIVE_LEFT)
			    queue.enqueue_now_P("M104 T0 S280");
            else
                queue.enqueue_now_P("M104 T1 S280");
			targetTemp = 280;
		}
			...
			if (gltouchpara.value == KEY_FILAMENT_BUTTON_RETURN)
			{
                if (wt_machineStatus != WS_PAUSE)
                {
                    ...
                    else
                    {
                        ...
                        queue.enqueue_now_P("M410");
						queue.enqueue_now_P("M18");
						queue.enqueue_now_P("T0 S"); //X40 PRO
                        Goback();
                    }
                }
                else
